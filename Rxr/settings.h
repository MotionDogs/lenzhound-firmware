...
inline int settings_get_motor_driver()
{
    if (eeprom_read_uint16(SENTINEL_LOC) != SENTINEL_VALUE) {
        settings_reset_to_defaults();
        return DEFAULT_MOTOR_DRIVER;
    }
    return eeprom_read_int16(MOTOR_DRIVER_LOC);
}

inline void settings_set_motor_driver(int val)
{
    return eeprom_write_int16(MOTOR_DRIVER_LOC, val);
}
